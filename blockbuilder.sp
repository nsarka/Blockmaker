...
public OnMapStart()
{
	...
	AddFileToDownloadsTable("sound/blockbuilder/nuke.mp3");
...
//		}
	...
	if(KeyValuesToFile(g_hBlocksKV, file)) {
		PrintToChatAll("\x03%s\x04 %d blocks and %d pair of teleporters were saved for map %s.", CHAT_TAG, blocks, teleporters, map);
		PrintToServer("%d blocks and %d teleports saved for map %s", blocks, teleporters, map);
	} else {
		PrintToChatAll("Could not save map blocks %s", map);
		PrintToServer("Could not save map blocks %s", map);
	}
...
SaveBlocks(bool:msg=false)
{
	...
	if(KeyValuesToFile(g_hBlocksKV, file)) {
		...
		PrintToChatAll("Could not save blocks for map %s", map);
		PrintToServer("Could not save blocks for map %s", map);
	}
}
		...
		} else if(g_iTeleporters[a]>1 && 2<=GetClientTeam(client)<=3)
		{
			...
			if(IsValidBlock(g_iTeleporters[a]))
			{
				if(fPos[0] - 32.0 < fPos2[0] < fPos[0] + 32.0 && fPos[1] - 32.0 < fPos2[1] < fPos[1] + 32.0 && fPos[2] - 64.0 < fPos2[2] < fPos[2] + 64.0)
				{
					...
					if(fVelocity[2] < 0.0) {
						fVelocity[2] *= -1.0; // Reverse z velocity
					}

					GetConVarString(g_hTeleSound, sound, sizeof(sound));
					GetEntPropVector(g_iTeleporters[a], Prop_Data, "m_vecOrigin", fPos2);
					TeleportEntity(client, fPos2, NULL_VECTOR, fVelocity); // Changed NULL_VECTOR in 3rd param to fVelocity to reverse z velocity
					EmitAmbientSound(TELE_SOUND_PATH, fPos2, client, SNDLEVEL_RAIDSIREN);
				}
			}
		}
	}
	...
	if(!IsFakeClient(client))
	{
		if(IsClientInGame(client))
		{
			...
			if(time > 0)
			{
				...
				PrintHintText(client, "%s will wear off in: %i", effectname, time)
...
stock GetCurrentWorkshopMap(String:szMap[], iMapBuf, String:szWorkShopID[], iWorkShopBuf)
{
    ...
    if(StrContains(szCurMap, "workshop", false) != -1) {
